...
public class AppServiceImpl implements AppService {
	...
	public boolean deletePost(Long postId, Long memberId) {
		...
		if(dao.checkPostId(postId, memberId)==1) {
			dao.deletePostImage(postId);
			dao.deletePostImageRelation(postId);
			dao.deletePostComment(postId);
			dao.deletePost(postId, memberId);
			return true;
		} 
		return false;
	...
	public void updatePost(Long postId, Long memberId, String content, ArrayList<Image> images, String[] deleteList) {

		iDaoMobile dao = sqlSession.getMapper(iDaoMobile.class);
		
		for(int i = 0 ; i < deleteList.length; ++i) {
			dao.deleteImageRelation(postId, deleteList[i]);
			dao.deleteImage(postId, deleteList[i]);
		}
		dao.updatePost(postId, memberId, content);
		if (images != null && images.size() > 0) {
			for (int i = 0; i < images.size(); ++i) {
				Image image = images.get(i);
				dao.insertImage(image);
				dao.relationPostImage(postId, image.getId());
			}
		}
	}
	...
	public void updateProfile(Image image, Long clubId, Long memberId) {
		...
		if (image != null) {
			dao.insertImage(image);
			tmpImgId = profile.getImage_id();
			profile.setImage_id(image.getId());
			dao.updateProfile(profile);
			dao.deleteProfileImage(tmpImgId);
		} else {
			dao.updateProfile(profile);
		}
